...
public class MongoDbCollectionEventStore<C extends Context> implements EventStore<C> {
    ...
    public EventStream<C> eventStream(String streamId) {
        ...
        final List<EventTransaction<C>> events = ImmutableList.copyOf(
            Iterators.transform(cursor,
                tx -> new EventTransaction<>(streamId, tx.getVersion(), tx.getEvents(), (C) tx.getContext())));
    ...
    public Version append(EventTransaction<C> tx, Version expectedVersion) {
        final String id = tx.getAggregateId();
